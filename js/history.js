export function filterAndSortRuns(runs, { query = "", shoeId = "all", sort = "newest", getShoeName }) {
  const normalized = query.trim().toLocaleLowerCase("de");
  return runs.filter((run) => {
    if (shoeId !== "all" && (run.shoeId || "") !== shoeId) return false;
    if (!normalized) return true;
    return [run.date, run.location, run.route, run.terrain, run.weather, run.notes, getShoeName(run.shoeId)]
